				...
				.each(function() {
					...
					this.createHtml = function() {
						...
						if (para.dragDrop) {
							...
							html += "					</div>";
							html += "				</div>";
							html += '				<span id="fileDragArea" class="upload_drag_area">或者拖动文件到此处</span>';
							html += "			</div>";
								...
								.bind(
										...
										function(e) {
											if (ZYFILE.funReturnNeedFiles().length > 0) {
												pano_name = document
														.getElementById("pano_name").value;
												pano_type = document
														.getElementById("pano_type").value;
												pano_info = document
														.getElementById("pano_info").value;
												// alert("pano_name=" +
												// pano_name);
												if (pano_name == ""
														|| pano_type == ""
														|| pano_info == "") {
													alert("请把信息填写完成后再上传,文件数量为:"
															+ ZYFILE
																	.funReturnNeedFiles().length);
												} else {
													// 设置进度条
													document
															.getElementById("greypage").style.display = "block";
													// 传入用户输入的标题
													ZYFILE.url = "UploadAction.servlet?name="
															+ pano_name
															+ "&&username="
															+ username;
													$("#fileSubmit").click()
